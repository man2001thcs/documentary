...
Cài đặt K8S và Rancher trên một server là một quy trình tương đối đơn giản. Dưới đây là hướng dẫn chi tiết để bạn thực hiện.

## Bước 1: Cài đặt K8S

K8S rke2 là một phiên bản K8S của rancher, được thiết kế để đơn giản hóa việc triển khai. Để cài đặt K8S, bạn có thể làm theo các bước sau:
...
Lấy token trong đường link /var/lib/rancher/rke2/server/node-token. Đây là token sử dụng để adđ thêm node machine sau này.
...
Sử dụng token vừa lấy được ở bước trước, thay vào trong file này:
...
Khởi động node với cấu hình trên:

``` sh
systemctl start rke2-agent.service
``` 

Cấu hình KUBECONFIG: Để sử dụng kubectl với K8S, bạn cần cấu hình biến môi trường KUBECONFIG.
...
Cert-Manager là một yêu cầu cần thiết để Rancher có thể quản lý chứng chỉ SSL tự ký. Gõ lần lượt các lệnh sau:
...
## Bước 4: Cài đặt Nginx ingress controller (Phụ, thường gói cài đặt của rancher đã bao gồm NGINX, có thể bỏ qua nếu chưa có)
...
Quá trình này sẽ giúp bạn cài đặt và cấu hình K8S và Rancher trên một server duy nhất. Rancher sẽ cung cấp cho bạn một giao diện người dùng mạnh mẽ để quản lý các cluster Kubernetes và các ứng dụng chạy trên đó.
